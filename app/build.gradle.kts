...
tasks.register("printAllApkSizes") {
  doLast {
    val apk1 = file("build/outputs/apk/debug/app-debug.apk")
    val apk2 = file("../.build-outputs/app-debug.apk")
    val apk3 = file("../APK_DOWNLOAD/app-debug.apk")
    println("SIZE_1: ${if (apk1.exists()) apk1.length().toString() else "NOT_FOUND"}")
    println("SIZE_2: ${if (apk2.exists()) apk2.length().toString() else "NOT_FOUND"}")
    println("SIZE_3: ${if (apk3.exists()) apk3.length().toString() else "NOT_FOUND"}")
  }
}
